...
SELECT * FROM person
SELECT 
    COALESCE(ID, SSN, PASSPORTID, ITIN) AS non_null
FROM person
-----------------MEDIUM TASKS--------------
--1--
SELECT 
...
SELECT 
    ...
    100.0 * SUM(CASE WHEN DATEDIFF(YEAR, HireDate, GETDATE()) > 3 THEN 1 ELSE 0 END) / COUNT(*) AS PercentageOver3Years
FROM Employees
GROUP BY Department;
...
SELECT 
    SId,
    Score,
    SUM(Score) OVER (ORDER BY SId ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS RunningTotal
FROM Students;
--7--
...
JOIN (
    ...
    GROUP BY BirthDate
    HAVING COUNT(*) > 1
) Dup ON S1.BirthDate = Dup.BirthDate;
...
SELECT
    STRING_AGG(c, '') WITHIN GROUP (ORDER BY n) AS UppercaseLetters,
    STRING_AGG(cl, '') WITHIN GROUP (ORDER BY n) AS LowercaseLetters,
    STRING_AGG(d, '') WITHIN GROUP (ORDER BY n) AS Numbers,
    STRING_AGG(o, '') WITHIN GROUP (ORDER BY n) AS OtherChars
FROM (
    SELECT 
        n,
        CASE WHEN c COLLATE Latin1_General_CS_AS BETWEEN 'A' AND 'Z' THEN c END AS c,
        CASE WHEN c COLLATE Latin1_General_CS_AS BETWEEN 'a' AND 'z' THEN c END AS cl,
        CASE WHEN c BETWEEN '0' AND '9' THEN c END AS d,
        CASE WHEN c NOT LIKE '[A-Za-z0-9]' THEN c END AS o
    FROM InputStr
    CROSS APPLY (
        SELECT TOP (LEN(str)) 
            ROW_NUMBER() OVER (ORDER BY (SELECT NULL)) AS n,
            SUBSTRING(str, ROW_NUMBER() OVER (ORDER BY (SELECT NULL)), 1) AS c
        FROM sys.objects
    ) t
) x;
