...
SELECT 
    ...
    Last_Name,
    Hire_Date,
    DATEDIFF(YEAR, Hire_Date, GETDATE()) AS WorkingYears
FROM Employees
...
SELECT EMPLOYEE_ID,
	   ...
	   LAST_NAME,
	   Hire_Date,
	  DATEDIFF(YEAR, Hire_Date, GETDATE()) AS WORKING_YEARS,
	  CASE
...
SELECT 
    Vals,
    LEFT(Vals, PATINDEX('%[^0-9]%', Vals + 'a') - 1) AS [Integer]
FROM GetIntegers
...
SELECT
    STRING_AGG(CASE WHEN SUBSTRING(@str, n, 1) LIKE '[0-9]' THEN SUBSTRING(@str, n, 1) END, '') AS Integers,
    STRING_AGG(CASE WHEN SUBSTRING(@str, n, 1) LIKE '[A-Za-z]' THEN SUBSTRING(@str, n, 1) END, '') AS Characters
FROM Numbers
