...
SELECT CATEGORY, SUM(StockQuantity) AS PRODUCT_COUNT FROM PRODUCTS
GROUP BY CATEGORY
...
--8--
SELECT Country, COUNT(CustomerID) AS TOTAL_CUSTS_COUNTRY
FROM Customers
GROUP BY Country
--9--
...
FROM Orders
--10--
SELECT DISTINCT Order1.CUSTOMERID 
FROM Orders AS Order1
WHERE OrderDate >= '2023-01-01' AND Order1.OrderDate < '2023-02-01'
	  AND NOT EXISTS (SELECT 1 FROM Invoices AS invoice
...
--13--
SELECT OrderDate, AVG(TotalAmount) AS AMOUNT_BY_YEAR
FROM Orders
GROUP BY OrderDate
--14--
SELECT ProductName,
...
--20--
SELECT district_id, Year, Population
